...
def explain_failure(status, note: str) -> None:
    ...
    if status in (401, 403):
        ...
        print()
        print("  Recent Roku OS ships this as \"Limited\", which blocks third-party")
        print("  control. Set it to:")
        print()
        print("    Enabled     <- pick this. Same-network control, which is us.")
        print("    Permissive  - also allows control from other subnets. Only needed")
        print("                  if your laptop and TV are on different networks.")
        print()
        print("  Note ECP has no authentication, so once this is on, anything on")
        print("  your network can drive the TV. That is inherent to the protocol.")
        return
